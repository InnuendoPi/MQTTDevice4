...
#define MAX_BUFFER_LENGTH 20	   // 14
#define MAX_LIST_LENGTH 30		   // 24
#define ATTRIBUTE_TEXT_LENGTH 90   // 48
#define ATTRIBUTE_TEXT_LENGTH_X 64 // 64
#define ATTRIBUTE_NUM_LENGTH 32	   // 32
#define ATTRIBUTE_NUM_LENGTH_X 48  // 48
#define RECEIVE_STRING_LENGTH 40   // 32

#define TIMEOUT 80

// color definitions
#define BLACKX2 0x0000
#define BLUEX2 0x001F
#define REDX2 0xF800
#define GREENX2 0x07E0
#define CYANX2 0x07FF
#define MAGENTAX2 0xF81F
#define YELLOWX2 0xFFE0
#define LIGHT_GREYX2 0xBDF7
#define GREYX2 0x8430
#define DARK_GREYX2 0x4208
#define WHITEX2 0xFFFF
...
void NextionComPort::update()
{
	...
	if ((length > 0) && (debugSerial != nullptr))
		dbgLoop();
	if ((length > 0) && (inputString[0] == 0x23))
	{
...
{
	if ((length > 0) && (inputString[0] == 0x23)) {
		debugSerial->write(" lastPageID ");
		debugSerial->println(lastPageID);
		debugSerial->write(" currentPageID ");
		debugSerial->println(currentPageID);
	}
	else if ((length == 1) && ((inputString[0] < 0x25) || (inputString[0] > 0x85))) {
